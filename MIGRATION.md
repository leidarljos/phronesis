...
|-----|-----|
| `GROKOS_POLICYD_STATE_DIR` | Passed as `state_dir` to `open` |
| `GROKOS_POLICYD_RUNTIME_DIR` | Passed as `runtime_dir` to `open` |
| `GROKOS_POLICYD_AGENT` | Agent id hex for Cap'n `AgentId` |
...
1. **Agent FFI** — `grokos-agent/src/policyd.rs`: `#[link(name = "grok_policyd")]`,
   `grok_supervisor_*` / `grok_policyd_*`, `GROKOS_POLICYD_STATE_DIR` /
   `GROKOS_POLICYD_RUNTIME_DIR` / `GROKOS_POLICYD_DENY_ALL` /
   `GROKOS_POLICYD_JANET_PACK`. Also `tool_policy.rs`, `pack_mailbox.rs`.
2. **Session launch** — `grokos-session/crates/grokos-sessiond/src/policyd_ffi.rs`
   ...
   `GROKOS_POLICYD_*` / `GROKOS_STATE_DIR` into the seat.
