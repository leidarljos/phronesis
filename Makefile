...
$(BUILD)/wire-%.o: src/wire/%.c $(SCHEMA_GEN_H) | $(BUILD)
	$(CC) $(WIRE_CPPFLAGS) $(CFLAGS) $(HOST_CFLAGS) -c -o $@ $<

$(BUILD)/wire-policy.capnp.o: $(SCHEMA_GEN_C) $(SCHEMA_GEN_H) | $(BUILD)
	$(CC) $(WIRE_CPPFLAGS) $(CFLAGS) -Wno-unused-parameter -c -o $@ $<

$(BUILD)/%.o: tests/%.c $(SCHEMA_GEN_H) | $(BUILD)
	$(CC) $(WIRE_CPPFLAGS) $(CFLAGS) $(CMOCKA_CFLAGS) $(HOST_CFLAGS) \
		-Wno-format-truncation -c -o $@ $<

# capnpc-c writes next to the .capnp; copy SoT into build/ then compile there.
# Outputs are under build/ only (gitignored). Cancel make's implicit .c → binary.
$(SCHEMA_GEN_C) $(SCHEMA_GEN_H): schema/policy.capnp | $(BUILD)
	...
	cp -f schema/policy.capnp $(BUILD)/policy.capnp
	cd $(BUILD) && capnp compile -o $$(command -v capnpc-c) policy.capnp
	test -f $(SCHEMA_GEN_C) && test -f $(SCHEMA_GEN_H)
...
# Writes build/policy.capnp.{c,h} only (SoT remains schema/policy.capnp).
schema: $(SCHEMA_GEN_C) $(SCHEMA_GEN_H)
