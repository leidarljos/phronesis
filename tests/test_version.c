...
{
	(void)state;
	assert_non_null(policyd_version_string());
	assert_string_equal(policyd_version_string(), POLICYD_VERSION);
	assert_int_equal(policyd_api_version(), POLICYD_API_VERSION);
	assert_int_equal(POLICYD_API_VERSION, 3);
}

/*
 * Catch VERSION/header drift: package major in the header must match the
 * first component of POLICYD_VERSION (SONAME key = package major).
...
{
	const char *v = POLICYD_VERSION;
	int major = 0;
	(void)state;
	assert_true(sscanf(v, "%d.", &major) == 1);
	assert_int_equal(major, POLICYD_VERSION_MAJOR);
}
