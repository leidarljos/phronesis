 ...
 * Build: make example  (links build/libgrok_policyd.a)
 */
...
int main(void)
{
	...
	if (make_dirs(state_dir, sizeof(state_dir), runtime_dir, sizeof(runtime_dir)) != 0) {
		fprintf(stderr, "mkdtemp failed (need writable /var/tmp or GROK_POLICYD_EXAMPLE_ROOT)\n");
		return 1;
	}
